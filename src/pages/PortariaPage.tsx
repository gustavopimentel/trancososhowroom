...
const imageModules = import.meta.glob('@/assets/images/novos-amenities/portaria/*.webp', { 
  eager: true, 
  import: 'default' 
}) as Record<string, string>

// Ordenar imagens pelo número no nome do arquivo
const portariaImages = Object.entries(imageModules)
  .map(([path, image]) => {
    // Extrair número do nome do arquivo (ex: "Portaria e Administracao 01" -> 1)
    const match = path.match(/Portaria e Administracao (\d+)/)
...
import legendaPortaria from '@/assets/images/novos-amenities/portaria/Grafismo_Legenda Portaria e Administracao.svg'
      ...
      >
        {images.map((image, index) => (
          <SwiperSlide key={index} className="!flex items-center justify-center">
            <img
              src={image}
              alt={`Portaria e Administração ${index + 1}`}
...
export function PortariaPage() {
  ...
  const handleNextSlide = () => {
    if (portariaImages.length > 0 && selectedImageIndex !== null) {
      const newIndex = (selectedImageIndex + 1) % portariaImages.length
  ...
  const handlePreviousSlide = () => {
    if (portariaImages.length > 0 && selectedImageIndex !== null) {
      const newIndex = (selectedImageIndex - 1 + portariaImages.length) % portariaImages.length
      setSelectedImageIndex(newIndex)
      setCurrentSlideIndex(newIndex)
    }
  }
  ...
  const handleGoToSlide = (index: number) => {
    if (portariaImages.length > 0 && index >= 0 && index < portariaImages.length) {
      setSelectedImageIndex(index)
      setCurrentSlideIndex(index)
    }
  }
    ...
    <div className="relative w-full h-full bg-white overflow-hidden flex">
      ...
      <div className="w-[20%] h-full flex flex-col items-center justify-center px-8 relative">
        ...
        >
          ...
          <h1 className="text-gray-900 font-serif text-3xl uppercase text-center leading-tight mb-12">
            Amenities Portaria e Administração
          </h1>
      ...
      <div className="w-[80%] h-full p-8 overflow-y-auto relative">
        <div className="grid grid-cols-4 gap-4 max-w-7xl mx-auto">
          {portariaImages.map((image, index) => (
            <motion.div
              key={index}

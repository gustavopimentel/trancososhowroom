...
const imageModules = import.meta.glob('@/assets/images/novos-amenities/Academia/*.webp', { 
  eager: true, 
  import: 'default' 
}) as Record<string, string>

// Ordenar imagens pelo número no nome do arquivo
const academiaImages = Object.entries(imageModules)
  .map(([path, image]) => {
    // Extrair número do nome do arquivo (ex: "Academia 01" -> 1)
    const match = path.match(/Academia (\d+)/)
...
import legendaAcademia from '@/assets/images/novos-amenities/Academia/Grafismo_Legenda Academia.svg'
...
export function AcademiaPage() {
  ...
  const handleNextSlide = () => {
    if (academiaImages.length > 0 && selectedImageIndex !== null) {
      const newIndex = (selectedImageIndex + 1) % academiaImages.length
  ...
  const handlePreviousSlide = () => {
    if (academiaImages.length > 0 && selectedImageIndex !== null) {
      const newIndex = (selectedImageIndex - 1 + academiaImages.length) % academiaImages.length
      setSelectedImageIndex(newIndex)
      setCurrentSlideIndex(newIndex)
    }
  }
  ...
  const handleGoToSlide = (index: number) => {
    if (academiaImages.length > 0 && index >= 0 && index < academiaImages.length) {
      setSelectedImageIndex(index)
      setCurrentSlideIndex(index)
    }
  }
    ...
    <div className="relative w-full h-full bg-white overflow-hidden flex">
      ...
      <div className="w-[80%] h-full p-8 overflow-y-auto relative">
        <div className="grid grid-cols-4 gap-4 max-w-7xl mx-auto">
          {academiaImages.map((image, index) => (
            <motion.div
              key={index}
